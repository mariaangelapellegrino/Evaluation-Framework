...
class ClusteringModel(AbstractModel):
    ...
    def train(self, merged, ignored):
        ...
        if n_samples < self.n_clusters:
            raise ValueError(
                (
                    "Clustering : Cannot have number of cluster n_clusters="
                    + str(self.n_clusters)
                    + " greater"
        ...
        trueLabels = pd.concat(
            [trueLabels, ignored["cluster"]], ignore_index=True, axis=0
        )
        trueLabels = trueLabels.values

        if labels.ndim != 1:
            raise ValueError("predicted labels must be 1D: shape is " + labels.ndim)
        if trueLabels.ndim != 1:
            raise ValueError("true labels must be 1D: shape is " + trueLabels.ndim)
        if labels.shape != trueLabels.shape:
            raise ValueError(
                "true labels and predicted labels must have same size, got "
                + str(len(trueLabels))
                + " and "
                + str(len(labels))
            )
        ...
        return {
            ...
            "adjusted_mutual_info_score": round(
                adjusted_mutual_info_score, float_precision
            ),
            ...
            "normalized_mutual_info_score": round(
                normalized_mutual_info_score, float_precision
            ),
    ...
    def _compute_clustering_accuracy(y_true, y_pred) -> float:
        ...
        if y_pred.size != y_true.size:
            raise Exception(
                "True and predicated labels for clustering accuracy have to be of the same size."
            )
        ...
        return (
            float(sum([w[i, j] for i, j in zip(row_indices, col_indices)]))
            / y_pred.size
        )
