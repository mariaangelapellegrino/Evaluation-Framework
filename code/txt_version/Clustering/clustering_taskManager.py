...
from clustering_dataManager import DataManager
...
class ClusteringManager (AbstractTaskManager):
        ...
        for i in range(len(gold_standard_filenames)):
            ...
            if data.size == 0:
                ...
                for model_name in clustering_models:
                    model = Model(model_name, metric = self.distance_metric, n_clusters=n_clusters, debugging_mode = self.debugging_mode)
    ...
    def storeResults(self, results_folder, gold_standard_filename, scores):
        with open(results_folder+'/clustering_'+gold_standard_filename+'_results.csv', "a+") as csv_file:
