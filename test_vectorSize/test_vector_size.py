...
def generate_cropped_files():
	...
	vectors_df = pd.read_csv(vector_filename, "\s+",  names=headers,  encoding='utf-8', index_col=False)
...
def evaluate_cropped_files():
	...
	for filename in vectors_file:
		...
		evaluation_manager.evaluate(filename, vector_size=vector_size, tasks=['Regression'], parallel=True,  debugging_mode = False)
